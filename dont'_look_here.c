...
// Avaliação simples (sem comentários ainda)
void avaliarFilme() {
    int id;
    float nota;
    ...
    printf("Sua nota para %s (0 a 5): ", filmes[id].nome);
    scanf("%f", &nota);
    getchar();
    ...
    printf(GREEN "Avaliação registrada!\n" RESET);
}

// Admin adiciona filme
void adicionarFilme() {
    if(totalFilmes >= 50){
        printf(RED "Limite atingido!\n" RESET);
        return;
    }

    printf("Nome do filme: ");
    fgets(filmes[totalFilmes].nome, 50, stdin);

    printf("Resumo: ");
    fgets(filmes[totalFilmes].resumo, 200, stdin);

    filmes[totalFilmes].nota = 0;

    totalFilmes++;

    printf(GREEN "Filme adicionado!\n" RESET);
}
...
// Carregar 20 filmes iniciais com resumo
void inicializarFilmes() {

    strcpy(filmes[0].nome, "Matrix\n");
    strcpy(filmes[0].resumo, "Um hacker descobre que o mundo é uma simulação.\n");
    filmes[0].nota = 4.8;

    strcpy(filmes[1].nome, "Interestelar\n");
    strcpy(filmes[1].resumo, "Um grupo viaja pelo espaço em busca de um novo lar.\n");
    filmes[1].nota = 4.9;

    strcpy(filmes[2].nome, "Vingadores: Ultimato\n");
    strcpy(filmes[2].resumo, "Os heróis lutam para desfazer o estalo de Thanos.\n");
    filmes[2].nota = 4.7;

    strcpy(filmes[3].nome, "O Senhor dos Aneis: Retorno do Rei\n");
    strcpy(filmes[3].resumo, "Frodo tenta destruir o Um Anel.\n");
    filmes[3].nota = 4.9;

    strcpy(filmes[4].nome, "Coringa\n");
    strcpy(filmes[4].resumo, "A origem do vilão mais famoso de Gotham.\n");
    filmes[4].nota = 4.6;

    strcpy(filmes[5].nome, "Gladiador\n");
    strcpy(filmes[5].resumo, "Um general busca vingança como escravo gladiador.\n");
    filmes[5].nota = 4.8;

    strcpy(filmes[6].nome, "O Rei Leão\n");
    strcpy(filmes[6].resumo, "Simba precisa assumir seu destino no reino animal.\n");
    filmes[6].nota = 4.7;

    strcpy(filmes[7].nome, "Titanic\n");
    strcpy(filmes[7].resumo, "Um romance trágico no navio mais famoso do mundo.\n");
    filmes[7].nota = 4.5;

    strcpy(filmes[8].nome, "A Origem\n");
    strcpy(filmes[8].resumo, "Um ladrão invade sonhos em missões perigosas.\n");
    filmes[8].nota = 4.8;

    strcpy(filmes[9].nome, "Forrest Gump\n");
    strcpy(filmes[9].resumo, "Um homem simples vive grandes momentos da história.\n");
    filmes[9].nota = 4.9;

    strcpy(filmes[10].nome, "Avatar\n");
    strcpy(filmes[10].resumo, "Um soldado vive entre alienígenas em Pandora.\n");
    filmes[10].nota = 4.6;

    strcpy(filmes[11].nome, "Os Incríveis\n");
    strcpy(filmes[11].resumo, "Uma família de super-heróis tenta viver normalmente.\n");
    filmes[11].nota = 4.5;

    strcpy(filmes[12].nome, "Homem-Aranha\n");
    strcpy(filmes[12].resumo, "Peter Parker ganha poderes após ser picado.\n");
    filmes[12].nota = 4.7;

    strcpy(filmes[13].nome, "Batman: O Cavaleiro das Trevas\n");
    strcpy(filmes[13].resumo, "Batman enfrenta o caos do Coringa.\n");
    filmes[13].nota = 4.9;

    strcpy(filmes[14].nome, "Pantera Negra\n");
    strcpy(filmes[14].resumo, "T'Challa protege Wakanda de ameaças internas.\n");
    filmes[14].nota = 4.6;

    strcpy(filmes[15].nome, "Duna\n");
    strcpy(filmes[15].resumo, "Paul enfrenta uma guerra no deserto do planeta Arrakis.\n");
    filmes[15].nota = 4.5;

    strcpy(filmes[16].nome, "Top Gun: Maverick\n");
    strcpy(filmes[16].resumo, "Um piloto treina jovens recrutas.\n");
    filmes[16].nota = 4.7;

    strcpy(filmes[17].nome, "Toy Story\n");
    strcpy(filmes[17].resumo, "Brinquedos ganham vida quando ninguém está olhando.\n");
    filmes[17].nota = 4.8;

    strcpy(filmes[18].nome, "Procurando Nemo\n");
    strcpy(filmes[18].resumo, "Um pai atravessa o mar para achar seu filho.\n");
    filmes[18].nota = 4.7;

    strcpy(filmes[19].nome, "Wall-E\n");
    strcpy(filmes[19].resumo, "Um robô solitário muda o destino da humanidade.\n");
    filmes[19].nota = 4.8;
